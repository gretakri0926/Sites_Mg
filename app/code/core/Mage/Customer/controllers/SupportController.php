 ...
 */
class Mage_Customer_SupportController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    // Contact Sales Info from customer/support/sales form
    public function contactSalesAction()
    {
        if ($this->getRequest()->isPost()) {
            ...
            $req = $this->getRequest();
            $params['subject'] = ($req->getParam('subject'));
            $params['sales_rep_id'] = '6';
            $params['sender_id'] = ($customer->getId());
            $params['comment'] = ($req->getParam('comment'));
            $params['message_type'] = ($req->getParam('message_type'));

            // mysql insert
            try {
                $connection = Mage::getSingleton('core/resource')->getConnection('core_write'); 
                $insert_query = $connection->query("INSERT INTO sales_person_notifications (subject, sales_rep_id, sender_id, comment, message_type, date_created) VALUES ('$params[subject]', '$params[sales_rep_id]', '$params[sender_id]', '$params[comment]', '$params[message_type]', NOW())");
                $message = 'Your Message was sent successfully.  A response will be sent to ' . $customer->getEmail();
            } catch (Exception $e) {
                $message = $e;
            }
        } 
    ...
    // Get Sales Person's Contact Info
    public function salesAction()
    {
        ...
        $block = $this->getLayout()->getBlock('support_info');
        if ($block) {
        ...
        $this->getLayout()->getBlock('head')->setTitle($this->__('Sales Person Information'));
