...
JustGage = function(config) { 
  ...
  this.config = 
  {
    ...
    valueFontColor : (config.valueFontColor) ? config.valueFontColor : "#666",
...
//    "fill": getColorForPercentage((this.config.value - this.config.min) / (this.config.max - this.config.min), this.config.levelColors, this.config.levelColorsGradient),  
  ...
  this.txtValue. attr({
    ...
    "color":"#232F7C",
    "font-weight":"bold",
    "font-family":"karbon_semiboldregular",
    "fill":"#232F7C",
    "fill-opacity":"1"      
  });
  ...
  //console.log(this.config.value+" "+this.config.min+" "+this.config.max+" "+this.params.widgetW+" "+this.params.widgetH+" "+this.params.dx+" "+this.params.dy+" "+this.config.gaugeWidthScale);
  
  var this_new = this;
  var value_new = this.config.value;
  
  this.level.animate({pki: [this.config.max, this.config.min, this.config.max, this.params.widgetW, this.params.widgetH,  this.params.dx, this.params.dy, this.config.gaugeWidthScale]},  this.config.startAnimationTime, this.config.startAnimationType, function(){
		this_new.refresh(value_new);
  });
